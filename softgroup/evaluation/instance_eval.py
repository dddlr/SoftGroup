...
class TreesEval(ScanNetEval):
    """Similar to ScanNetEval, but using this instance label format:
    == 0: tree
    > 0: not tree
    """
            ...
            for oi, iou_th in enumerate(ious):
                ...
                for li, label_name in enumerate(self.eval_class_labels):
                    ...
                    for m in matches:
                        ...
                        gt_instances = [
                            gt for gt in gt_instances
                            if gt['instance_id'] >= 0 and gt['vert_count'] >= min_region_size and
                            gt['med_dist'] <= distance_thresh and gt['dist_conf'] >= distance_conf
                        ]
                        if gt_instances:
                            has_gt = True
    ...
    def assign_instances_for_scan(self, preds, gts):
        ...
        num_pred_instances = 0
        # mask of void labels in the groundtruth
        bool_void = np.logical_not(np.in1d(np.clip(gts, None, 1), self.valid_class_ids))
        # go thru all prediction masks
        for pred in preds:
            if self.use_label:
                label_id = pred['label_id']
